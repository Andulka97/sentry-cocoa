...
- (void)testCPUBenchmark
{
    ...
    // TODO: add @"Render image", @"Scroll UITableView", @"Network download", @"Network upload", @"Network stream up", @"Network stream down", @"Network stream mixed", @"WebKit render", @"Load empty DB", @"Load DB with entities", @"Create entity", @"Fetch entity", @"Update entity", @"Delete entity", @"Data encrypt"
    for (NSString *benchmark in @[@"File write", @"File read", @"File copy", @"File delete", @"CPU work", @"CPU idle", @"Data compression", @"Data SHA1 sum", @"JSON Encode", @"JSON Decode"]) {
...
- (void)performSelectedBenchmark:(NSString *)benchmark {
    ...
    for (NSUInteger j = 0; j < kSentryBenchmarkIterations; j++) {
        ...
        // After navigating to the test, the test app will do some work. Meanwhile the profiler does its thing, and the benchmarking observation in the test app records how much CPU time is used on the profiler thread as well as the rest of the app.
        ...
        NSLog(@"[Sentry Benchmark] [%@] %ld,%ld,%ld,%ld", benchmark, (long)profilerSystemTime, (long)profilerUserTime,
